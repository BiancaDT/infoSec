...
def SSHLogin(host,port,username,password):
    try:
        ...
        if ssh_session:
            print("SSH login succesful on %s: %s with username %s and passowrd %s" % (host, port, username, password))
...
def TelnetLogin(host,port,username,password):
    ...
    try:
        ...
        if (result[0] >=0):
            print("Telnet login succesful on %s: %s with username %s and password %s" % (host, port, username, password))
        tn.close()
    except EOFError:
